...
public class BabelCraft extends JavaPlugin {
	...
	{	
		...
		if(Config.plugin_usagestats)
		{
			...
			try { Util.PostInfo(getServer().getName(),getServer().getVersion(),Config.pluginversion,System.getProperty("os.name"),System.getProperty("os.version"),System.getProperty("os.arch"),System.getProperty("java.version"),"","",Plugins,online,max,Config.Server.getPort()); } 
			catch (IOException e1) { Util.Debug("Could not send data to main server."); }		
		}
	...
	public static String Translate(String message, Player player)
	{
		...
		else
		{
			...
			if(isin) { lang = Util.GetPlayerLanguageHash(player); }
			else { lang = Util.GetLanguage(player,"to"); }
		}
	...
	public static String getLanguage(Player player)
	{
		...
		Language lang;
		if(isin) { lang = Util.GetPlayerLanguageHash(player); }
		else { lang = Util.GetLanguage(player,"from"); }
	...
	public static String getCountryCode(Player player)
	{
		boolean isin = Util.PlayerDatabase("check", player, null, null);
		String CountryCode;
		if(isin) { CountryCode = Util.GetPlayerCountryHash(player); }
		else {  CountryCode = Util.GetCountryCode(Util.GetIP(player)); }
