...
public class BabelCraftPlayerListener extends PlayerListener
{
  ...
  public BabelCraftPlayerListener(BabelCraft instance)
  {
   ...
   if(Config.plugintag) { TempPrefix = Config.plugin_prefix; }
   else { TempPrefix = Config.nextline_tag; }
  }
  ...
  public void onPlayerJoin(PlayerJoinEvent event)
  {
	  ...
	  //else {   BabelCraft.ToFile("remove", event.getPlayer().getName().toLowerCase(),null); }
  }
  ...
  public void onPlayerCommandPreprocess(PlayerCommandPreprocessEvent event)
  {
    ...
    if (split[0].equals("/language") || split[0].equals("/lang")) 
    {
    	if(split.length == 2 || split.length == 3)
    	{
    		...
    		else if(parameter1.equals("search") || parameter1.equals("s"))
    		{
    	    	if(split.length == 3)
    	    	{
    				...
    				if(counter == 0) { player.sendMessage(TempPrefix+"Could not find any languages for string '"+parameter2+"'");}
    	    	}
    	    	else { player.sendMessage(Config.plugin_prefix+"Correct usage is: /lang search <String>"); }
    		}
    	...
    	else
    	{
			...
			Language lang;
			if(isin) { lang = Util.GetPlayerLanguageHash(event.getPlayer()); }
			else { lang = Util.GetLanguage(event.getPlayer(),"to"); }
  ...
  public void onPlayerChat(PlayerChatEvent event)
  {
	  if(!event.isCancelled())
	  {
		  boolean isin = Util.PlayerDatabase("check", event.getPlayer(),null, null);
		  Language langfrom;
		  if(isin) { langfrom = Util.GetPlayerLanguageHash(event.getPlayer()); }
		  else { langfrom = Util.GetLanguage(event.getPlayer(),"from"); }
		 // int counter = 0;
			      ...
			      for (String theplayer : players)  
			      {
			    	  ...
			    	  for (Player player : Config.Server.getOnlinePlayers()) 
					  {
			    		  if(player.getName().equals(theplayer))
			    		  {
							  ...
							  Language langto;
							  if(isin2) { langto = Util.GetPlayerLanguageHash(player); }
							  else { langto = Util.GetLanguage(player,"to"); }
